...
export default function CreateProduct() {
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...form,
      // Convert price to float, handling the case where it might be empty or invalid
      price: form.price ? parseFloat(form.price) : 0,
      isSubscription: Boolean(form.isSubscription),
      isPopular: Boolean(form.isPopular), // Ensure boolean conversion
      active: Boolean(form.active),     // Ensure boolean conversion
      // Process subscriptionTiers only if it's a subscription product
    ...
    // Conditionally add stripePriceId to payload if it's a subscription
    if (!payload.isSubscription) {
      delete payload.stripePriceId; // Remove if not a subscription
    } else if (payload.isSubscription && !payload.stripePriceId) {
        // Optional: Add validation for stripePriceId if isSubscription is true
        toast.error(t('messages.stripePriceIdRequired'));
        setIsLoading(false);
        return;
    }
  ...
  return (
    <div className="p-6 md:p-8 max-w-2xl mx-auto">
      <Card className="shadow-md">
        <CardHeader>
          <CardTitle className="text-2xl font-bold">{t('createProductTitle')}</CardTitle>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="md:col-span-2 border-t pt-4">
              ...
              <p className="text-sm text-gray-500 mt-2">
                Add pre-created checkout URLs from your payment providers. Create Payment Links in Stripe Dashboard and checkout URLs in LemonSqueezy.
              </p>
            </div>
            ...
            <Button type="submit" disabled={isLoading} className="w-full md:col-span-2 mt-4">
